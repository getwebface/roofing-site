...
function getPageData_(slug, pageType, ctx, debug) {
  ...
  let foundRow = null;
  let foundRowIndex = -1;
  let actualPageType = pageType; // Default to requested type
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    // Match on slug only if pageType is "auto"
    if (rowSlug === slug && (pageType === "auto" || rowType === pageType)) {
      foundRow = row;
      foundRowIndex = r + 1; // 1-based row number for humans
      
      // CRITICAL: Update local variable to the actual type found in the sheet
      // so it can be returned to the Worker for telemetry/logic.
      actualPageType = rowType;
      break;
    }
  }
  ...
  return debug
    ? {
        ...
        exists: true,
        slug,
        pageType: actualPageType, // Return actual type found
        copy: canonicalCopy,
        raw: rawCopy,
        debug: {
    ...
    : { success: true, exists: true, slug, pageType: actualPageType, copy: canonicalCopy, raw: rawCopy };
}
 ...
 * so HTML will render as text (safe). If you later want rich HTML slots, you’ll need
